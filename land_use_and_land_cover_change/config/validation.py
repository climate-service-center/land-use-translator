...
from lut_config import datadir, mcg, scenario_dict, scriptsdir, tf_file_syear, th_file_syear
...
schema = {
    "region": {
        "type": "string",
        "allowed": ["Australasia", "Europe", "Germany", "Global", "NorthAmerica", "WestAfrica"],
    },
...
def validate_config(config) -> None:
    ...
    valid schema is given in the variable `schema`.
    """
    # initialize custom validator
    v = CustomValidator(schema)

    # run custom validator on config
    v.validate(config)

    # output validation results and check for other problems in config
    if v.errors:
    ...
    if isinstance(config.n_jobs, int) and config.n_jobs > os.cpu_count():
        config.n_jobs = os.cpu_count()
        raise Warning(
            f"Number of jobs (n_jobs) must be less than or equal to the number of available CPU cores ({os.cpu_count()}). Setting n_jobs to {config.n_jobs}."
        )
...
def validate_pfts_file(namelist, config):
    ...
    try:
        ...
        raise KeyError(
            f"Year {year} not found in file. Check if the file has the correct time dimensions or if syear/eyear are correctly set."
        )
    if config.xsize != ds.sizes.get(x_dim) or config.ysize != ds.sizes.get(y_dim):
        raise ValueError(
            f"Wrong sizes given. Expected {config.xsize}x{config.ysize} but got {ds.sizes.get(x_dim)}x{ds.sizes.get(y_dim)}. Check if the sizes coincide with the correspondig grid_reg file located in {scriptsdir}/."
        )
...
def validate_main_files(namelist, config):
    ...
    if config.scenario in ["historical", "historical_high", "historical_low"]:
        ...
        afile = (
            f"added_tree_cover_input4MIPs_landState_ScenarioMIP_UofMD-{scenario_dict[config.scenario]}-2-1-f_gn_2015-2100.nc"
            if not config.path_file_addtree
            else config.path_file_addtree
        )
        sfile = (
            f"multiple-states_input4MIPs_landState_ScenarioMIP_UofMD-{scenario_dict[config.scenario]}-2-1-f_gn_2015-2100.nc"
            if not config.path_file_states
            else config.path_file_states
        )
        tfile = (
            f"multiple-transitions_input4MIPs_landState_ScenarioMIP_UofMD-{scenario_dict[config.scenario]}-2-1-f_gn_2015-2100.nc"
            if not config.path_file_trans
            else config.path_file_trans
        )
        mfile = (
            f"multiple-management_input4MIPs_landState_ScenarioMIP_UofMD-{scenario_dict[config.scenario]}-2-1-f_gn_2015-2100.nc"
            if not config.path_file_manag
            else config.path_file_manag
        )
    if config.prepare_mcgrath:
        ifile = f"{mcg}_{config.syear}_{config.mcgrath_eyear}.nc"
        validate_path(
            ifile, datadir, add_message="Consider adding the missing file or unable option 'prepare_mcgrath'."
        )
...
def validate_mcgrath_prepared_files(namelist, config):
    for key, value in namelist.items():
        if key == "F_MCGRATH":
            validate_path(
                value,
                add_message="Consider enabling the option 'prepare_mcgrath' to prepare the missing file or unable 'mcgrath'.",
            )
...
def validate_prepared_files(namelist, config):
    for key, value in namelist.items():
        ...
        if key == "F_GRID":
            ...
            key not in ["F_IRRI_IN", "F_ADDTREE", "F_MCGRATH", "F_LC_OUT", "F_LC_IN"]
            and not key.startswith("F_BACK")
            and not key.startswith("F_GLOBAL")
        ):
            validate_path(
                value, add_message="Consider enabling the option 'prepare_luh2_data' to prepare the missing file."
            )
        # checking file sizes

        if (
            key not in ["F_IRRI_IN", "F_ADDTREE", "F_MCGRATH", "F_GRID", "F_LC_OUT", "F_LC_IN"]
            and not key.startswith("F_BACK")
            and not key.startswith("F_GLOBAL")
        ):
            validate_dimensions(value, config)
...
def validate_dimensions(value, config, type=1):
    ...
    if not (ds.sizes.get("x") or ds.sizes.get("lon") or ds.sizes.get("rlon")) and not (
        ds.sizes.get("y") or ds.sizes.get("lat") or ds.sizes.get("rlat")
    ):
    ...
    if type == 2:
        if ds.sizes.get(x_dim) < config.xsize or ds.sizes.get(y_dim) < config.ysize:
            raise ValueError(
                f"File {value} has wrong dimensions. Expected {config.xsize}x{config.ysize} but got {ds.sizes.get(x_dim)}x{ds.sizes.get(y_dim)}"
            )
    else:
        if ds.sizes.get(x_dim) != config.xsize or ds.sizes.get(y_dim) != config.ysize:
            raise ValueError(
...
def validate_timerange(input_file, config):
    ...
    ds = xr.open_dataset(input_file, decode_times=False).sel(
        time=slice(config.syear - init_year, config.eyear - init_year)
    )
    if not ds.time.units.split()[2].split("-")[0] == str(init_year):
        raise ValueError(
            f"File {input_file} has wrong time dimensions. Did not find time range {config.syear}/{config.eyear}.Check if the transitions file belong to another scenario."
        )
