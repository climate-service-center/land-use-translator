"""
configuration file specifying general settings the land-use translator runs with
"""
...
mcgdir = f"{datadir}/MCGRATH"  # directory of McGrath forest fraction file, dataset not publicly available
plotdir = "plots"

# define CWT for LUH v2 states
URB = ["urban"]  # urban
...
# Variable definitions used in McGrath data
TeBrEv = 10
...
oname = "LUCAS_LUC7_LANDMATE_LUH2"
lcd = "LSM"  # version of ESA-file
mcg = "combined_species_mtc"  # name of McGrath file
vers = "v2.0.7"  # version of the model

# LUT expects data in the same format as the LANDMATE data (see https://doi.org/10.26050/WDCC/LM_PFT_GLOB_v1.2), i.e., there is one variable, usually called landCoverFrac, with three dimensions (lon, lat, pft_class) containing the land use fractions
# the following defines, which (1-based) indices of PFTs in the input data are set as the following PFT categories:
# Forest PFTS (FORPFTS)
# Shrub PFTS (SHRPFTS)
# Grass PFTS (GRAPFTS)
# Crop PFTS (CROPFTS)
# Urban PFTS (URBPFTS)
# Constant PFTS (CONPFTS)

## LANDMATE PFT assigments to LUH2 PFT categories
...
# the following defines the indices of the default PFTs (in LANDMATE format) for the different land use classes in LUH2
## LANDMATE defaults
GRADEF = 9
CRODEF = 13
SHRDEF = 8
...
vars_pfts = ""
for i in range(1, 17):
    vars_pfts += f"var{800 + i},"
vars_pfts = vars_pfts[:-1]  # remove trailing comma
...
# LANDMATE classes
# TODO: needs to be adapted to ESA-CCI classes and automatized to be generated from the dataset
output_file_comment = """
